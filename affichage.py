...
class Affichage():
    ...
    def charge_tuto(self):
        ...
        self.render_1 = self.font.render("Déplacez vous avec les touches :",False,(60, 65, 56))
        self.render_2 = self.font.render("[haut] [bas] [droite] [gauche]",False,(60, 65, 56))
        self.render_3 = self.font.render("Vous récuperez 10 sec lorsque",False,(60, 65, 56))
        self.render_4 = self.font.render("vous attraper un sablier.",False,(60, 65, 56))
        self.render_5 = self.font.render("Quitter la partie à tout instant",False,(60, 65, 56))
        self.render_6 = self.font.render("avec [echap]",False,(60, 65, 56))
        self.render_7 = self.font.render("La flèche à coté du personnage",False,(60, 65, 56))
        self.render_8 = self.font.render("indique toujours la sortie.",False,(60, 65, 56))
    ...
    def bonus(self) -> None:
        """Verifie si le joueur se trouve sur une case avec un bonus"""
    ...
    def camera(self) -> None:
        """Si le joueur depasse la zone de camera 2*2
        au centre de l'ecran, on decale la camera"""
        if not self.tuto:
    ...
    def time(self) -> bool:
        """ecrit le temps restant sur la fenetre, renvoie si le temps est finit"""
    ...
    def angle_boussole(self) -> float:
        '''Renvoie la direction de la sortie en degrée par rapport à la sortie'''
        y2,x2 = self.player.position
        y1,x1 = self.sortie
        return 180-atan2((y2-y1),(x2-x1))/pi*180

    def boussole(self) -> pygame.sprite:
        """renvoie une bousssole en format png tourné vers la sortie"""
        return pygame.transform.rotate(self.sprite_boussole, self.angle_boussole()-90)

    def update(self) -> None:
        """affiche le labyrinthe à un moment donné
        ne se met pas a jour, à chaque tour de boucle"""
        ...
        if self.zoom == 1:
            ...
            j,i = self.point_vision
            for y,x in self.murs_ver: #dessins des murs verticaux
            ...
            for y,x in self.murs_ori: #dessins des murs orizontaux
                if j-2 <= y <= j+2 and i-2 <= x <=i+2:
            ...
            #pygame.draw.circle(self.screen,(255, 0, 0),(self.screen.get_width()/2+((x-i)*dist_oriz)+dist_oriz/2,self.screen.get_height()/2+((y-j)*dist_vert)+dist_vert/2),court_oriz)
...
class Player():
    def __init__(self,position) -> None:
        """classe joueur qui comprent ses coordonnées en (y,x)"""
        self.position = position
    def haut(self):
        """modifie les coordonnées y-1"""
